...
import { CreateTaskRequest, PaginatedTasksResponse, Task, UpdateTaskRequest } from 'frontend/types/task';

export default class TaskService extends APIService {
  getTasks = async (accountId: string, page: number = 1, size: number = 10): Promise<ApiResponse<PaginatedTasksResponse>> => {
    const response = await this.apiClient.get(`/accounts/${accountId}/tasks?page=${page}&size=${size}`);
